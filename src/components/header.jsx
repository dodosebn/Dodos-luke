...
return () => window.removeEventListener('resize', OnResize);
  ...
  return (
    ...
    <div className="z-10 relative">
     <section className='grid grid-cols-2 w-full items-center gap-4 p-4'>

        <article>
          <div className={`relative  ${isMobile ? 'left-8' : 'left-0'}`}>
            <CiSearch
              fontSize={20}
              className="text-gray-400 absolute top-4 transform -translate-y-2/4 left-3"
            />
            <input
              type="text"
              placeholder="Search..."
        ...
        <article className="fixed top-3 left-0 right-7 z-20">
                    <div className="flex items-center justify-end md:gap-4 -z-40">
            <Popover
              className="relative"
              open={isOpen}
