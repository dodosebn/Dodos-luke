...
const DivHolder = ({ children }) => {
  return (
    <div className="relative flex flex-col items-center justify-center w-full p-2 md:p-12 bg-white rounded-sm border border-gray-200 ">
      {children}
    </div>
  );
};
          ...
          <DivHolder key={item.id}>
            <div className="flex justify-start mb-4">
              <div className="md:absolute left-6">
                <span className="rounded-full h-7 w-7 md:h-12 md:w-12 flex items-center justify-center bg-sky-500 ">
                  {item.img}
                </span>
              </div>
              <strong className="text-sm text-gray-500 font-light pl-2 md:pl-11 whitespace-nowrap ">
                {item.title}
              </strong>
            </div>
            <div className="flex items-center md:absolute top-20 right-5 ml-10 md:ml-0">
              <strong className="text-xl text-gray-700 md:font-semibold">
                {item.Funds}
              </strong>
